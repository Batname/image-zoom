  ...
  .directive('zoom', function(){
    function link(scope, element, attrs){
      ...
      function moveMark(offsetX, offsetY){
        ...
        var condition = {
          maxLeft: offsetX - dx/2 < 0,
          maxRigth: offsetX > originalImg[0].width - dx/2,
          maxTop: offsetY - dy/2 < 0,
          topBottom: offsetY > originalImg[0].height - dy/2
        };

        if(!!_.find(condition, function(chr){return chr == true})) {
          mark.addClass('hide');
          zoomed.addClass('hide');
          console.log('it less then zero');
        } else {
          mark.removeClass('hide');
